...
What this demonstrates:
  ...
  6. Recording rules — pre-computed series vs raw histogram fan-out
  7. RED method summary + gauge leak protection demo
  8. Cardinality explosion simulation — why user_id labels cause OOM
...
def send_requests(count, workers=10):
    ...
    lock = threading.Lock()
    stats = {"ok": 0, "err": 0, "latencies": [], "by_endpoint": {e: 0 for e in endpoints}}
...
def main():
    ...
    section("OBSERVABILITY LAB — Introduction")
    print("""
...
""")
    ...
    print()
    print("  Traffic is split ~equally across endpoints (random.choice).")
    print("  With 25% of requests to /api/flaky and a 5% error rate there,")
    print("  the overall error rate = 5% × 25% = ~1.25% of all requests.")
    print("  A 99.9% SLO allows 0.1% errors — this burns budget at ~12.5×.")
    print()
    ...
    print(f"  Requests per endpoint:")
    for ep, cnt in sorted(stats["by_endpoint"].items()):
        print(f"    {ep:<20} {cnt:>4} requests  ({cnt/total*100:.0f}%)")
    print(f"  Latency (client-side measured, nearest-rank):")
    print(f"    p50 = {p50:.1f}ms")
    print(f"    p95 = {p95:.1f}ms")
    print(f"    p99 = {p99:.1f}ms")
    print(f"""
  Note: p50 is dominated by /api/fast and /api/flaky (~10-50ms).
  ...
  Client-side p99 ({p99:.0f}ms) may differ slightly from Prometheus's
  histogram_quantile result due to interpolation at bucket boundaries.
""")
  ...
  Difference between rate() and irate():
    rate(counter[5m])  = average per-second rate over a 5-minute window
                         smoothed, good for alerting (less noise)
    irate(counter[5m]) = instantaneous rate based on the last two data points
                         more responsive, better for dashboards watching live traffic
    Always use rate() for alert rules; irate() is too spiky for SLO evaluation.
""")

    # ── Phase 3: Histogram Bucket Internals ───────────────────────
    section("Phase 3: Histogram Bucket Internals (How histogram_quantile Works)")
        ...
        if total_count > 0:
            ...
            for labels, count in sorted_buckets:
                ...
                if count >= threshold_99:
                    ...
                    print()
                    print(f"  Interview insight: if your SLO is p99 < 200ms but your buckets")
                    print(f"  skip from 100ms to 500ms, Prometheus can only say 'between 100ms")
                    print(f"  and 500ms' — it cannot confirm whether the SLO is met.")
                    print(f"  flask_app.py uses buckets=[..., 0.100, 0.200, 0.300, 0.500, ...]")
                    print(f"  specifically so that the 200ms SLO boundary is a bucket edge.")
                    break
                prev_le = le_val
    else:
    ...
    # ── Phase 4: SLO Analysis ─────────────────────────────────────
    section("Phase 4: SLO Analysis")
  ...
  Multi-window SLO tracking requires multiple time ranges:
    Short window (5m)  — detects active incidents quickly
    Long window (30d)  — enforces the monthly contract
    Both must be tracked; a short spike doesn't deplete the monthly budget.
""")
...
""")
    ...
    # ── Phase 5: Error Budget Burn Rate ───────────────────────────
    section("Phase 5: Error Budget Burn Rate Analysis")
...
""")
    ...
    if err_rate is not None:
        ...
        if burn_rate >= 14.4:
            ...
            severity = "OK — burn rate under 1x (budget not being consumed)"
  ...
  +------------------+------------+-------------+-------------+
  | Page (critical)  | 14.4x      | 1h          | 5m          |
  | Page (high)      | 6x         | 6h          | 30m         |
  | Ticket (medium)  | 3x         | 3d          | 6h          |
  +------------------+------------+-------------+-------------+

  PromQL for 14.4x burn rate (99.9% SLO):
    (
  ...
  Interview insight: the 14.4x threshold is not arbitrary.
  30 days x 24 hours / 2 hours = 360; but only 5% of budget remains after
  the page, so: 1/0.05 x (1/360) = ~14.4. It is derived from the
  budget exhaustion time and acceptable detection latency.
""")

    # ── Phase 6: Recording Rules Demo ─────────────────────────────
    section("Phase 6: Recording Rules — Pre-computed SLO Series")
  ...
  Waiting for recording rules to evaluate (up to 30s)...
""")

    # Recording rules evaluate every 15s; wait for them rather than hoping they fired.
    rec_p99 = wait_for_prom_data(
        "job:http_request_duration_p99:rate1m",
        "recording rule job:http_request_duration_p99:rate1m",
        timeout=30
    )
...
""")
        ...
        raw_p99_ms = p99_prom * 1000 if p99_prom is not None else None
        rec_p99_ms = rec_p99 * 1000
        delta_note = ""
        if raw_p99_ms is not None:
            delta = abs(rec_p99_ms - raw_p99_ms)
            delta_note = f" (delta vs raw query: {delta:.1f}ms — same data, different eval time)"

        print(f"""
  Raw PromQL p99:         {fmt(p99_prom, 'ms', 1000)}
  Recording rule p99:     {rec_p99_ms:.1f}ms{delta_note}

  Both queries use identical expressions — the recording rule simply
  caches the result. Any small delta reflects different evaluation timestamps.
...
""")
    ...
    # ── Phase 7: RED Method Summary + Gauge Leak Demo ─────────────
    section("Phase 7: RED Method Summary + Gauge Leak Protection")
  ...
  Gauge leak failure mode (and how flask_app.py prevents it):
    BAD pattern (bare inc/dec):
        ACTIVE_CONNECTIONS.inc()
        handle_request()           # if this raises, dec() is never called!
        ACTIVE_CONNECTIONS.dec()   # skipped on exception -> gauge grows forever

    GOOD pattern (context manager in flask_app.py):
        with track_request(endpoint) as set_status:
            handle_request()       # exception here -> finally block ALWAYS runs
        # finally: ACTIVE_CONNECTIONS.dec()  <- guaranteed by Python's 'with'

    Without the context manager, a crash loop causes the active_connections
    gauge to grow monotonically, creating a false "traffic surge" signal that
    looks like overload. The on-call engineer pages the wrong team.
    Use Python 'with', Go 'defer', or Java try-finally for gauges.
""")

    # Show the current active_connections gauge value
    active_conns = prom_query("http_active_connections")
    if active_conns is not None:
        print(f"  Current http_active_connections gauge: {active_conns:.0f}")
        print(f"  (Should be 0 — all requests from Phase 1 have completed.)")
        if active_conns > 0:
            print(f"  WARNING: gauge is {active_conns:.0f} > 0 — possible leak if no traffic is active!")
    else:
        print(f"  http_active_connections gauge: N/A (Prometheus scrape pending)")
  ...
  OpenTelemetry note:
    ...
    W3C Trace Context header (traceparent) propagates trace IDs across services,
    enabling exemplars that link a Grafana histogram bucket to a Jaeger trace.
...
""")
    ...
    # ── Phase 8: Cardinality Explosion Simulation ─────────────────
    section("Phase 8: Cardinality Explosion Simulation (Why user_id Labels Kill Prometheus)")
...
""")
  ...
  Key rules to stay safe:
    ...
    6. Label explosion is irreversible without a Prometheus restart
       (or TSDB compaction). Audit cardinality in staging before prod.
       The cardinality audit PromQL above is safe to run on live Prometheus.
""")
